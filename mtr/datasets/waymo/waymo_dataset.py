...
class WaymoDataset(DatasetTemplate):
    ...
    def create_scene_level_data(self, index):
        ...
        if self.use_lidar:
            lidar_data = self.create_lidar_data_for_center_objects(
                center_objects=center_objects,
                lidar_infos=info["frame_points_xyz"],
                center_offset=self.dataset_cfg.get("CENTER_OFFSET_OF_MAP", (30.0, 0)),
            )
    ...
    def transform_trajs_to_center_coords(
        ...
        if rot_vel_index is not None:
            ...
            obj_trajs[:, :, :, rot_vel_index] = common_utils.rotate_points_along_z(
                points=obj_trajs[:, :, :, rot_vel_index].view(
    ...
    def generate_batch_polylines_from_map(
        ...
        """Generate batch polylines from map.
        ...
        # Shift points by one to create consecutive pairs
        sampled_points_shift = np.roll(sampled_points, shift=1, axis=0)

        # Concat points so that each point is now [end_x, end_y, start_x, start_y]
        buffer_points = np.concatenate(
            (sampled_points[:, 0:2], sampled_points_shift[:, 0:2]), axis=-1
        )  # [ed_x, ed_y, st_x, st_y]

        # Set the starting point of the first polyline to be the same as the ending point of the first polyline
        buffer_points[0, 2:4] = buffer_points[0, 0:2]

        # Find indices where the distance between consecutive points exceeds the threshold
        break_idxs = (
            np.linalg.norm(buffer_points[:, 0:2] - buffer_points[:, 2:4], axis=-1)
            > vector_break_dist_thresh
        ).nonzero()[0]

        # Split points into lines depending on threshold
        polyline_list = np.array_split(sampled_points, break_idxs, axis=0)

        # Generate polylines (and mask)
        ret_polylines = []
        ret_polylines_mask = []
    ...
    def create_map_data_for_center_objects(
        ...
        def transform_to_center_coordinates(
            ...
            # Update polyline xyz coordinates using center objects
            neighboring_polylines[:, :, :, 0:3] -= center_objects[:, None, None, 0:3]

            # Rotate along z-axis so that angle is consistent
            neighboring_polylines[:, :, :, 0:2] = common_utils.rotate_points_along_z(
                points=neighboring_polylines[:, :, :, 0:2].view(
                    num_center_objects, -1, 2
                ),
            ...
            neighboring_polylines[:, :, :, 3:5] = common_utils.rotate_points_along_z(
                points=neighboring_polylines[:, :, :, 3:5].view(
                    num_center_objects, -1, 2
                ),
                angle=-center_objects[:, 6],
            ).view(num_center_objects, -1, batch_polylines.shape[1], 2)
            ...
            neighboring_polylines = torch.cat(
                (neighboring_polylines, xy_pos_pre), dim=-1
            )
        ...
        # Generate polylines from points based on sample interval and distance threshold
        ...
        if len(batch_polylines) > num_of_src_polylines:
            ...
            center_offset_rot = torch.from_numpy(
                np.array(center_offset, dtype=np.float32)
        ...
        # Calculate the center of each polyline
        temp_sum = (
    ...
    def create_lidar_data_for_center_objects(
        self,
        center_objects,
        lidar_infos,
        center_offset,
        ...
        # Transform object coordinates by center objects
        def transform_to_center_coordinates(lidar_points):
            # Update xyz coordinates using center objects
            lidar_points[:, :, :, 0:3] -= center_objects[:, None, None, 0:3]

            # Rotate along z-axis so that angle is consistent
            lidar_points[:, :, :, 0:2] = common_utils.rotate_points_along_z(
                points=lidar_points[:, :, :, 0:2].view(num_center_objects, -1, 2),
                angle=-center_objects[:, 6],
            ).view(num_center_objects, lidar_points.shape[1], -1, 2)

            return lidar_points

        max_length = max(_.shape[0] for _ in lidar_infos.values())
        reshaped_arrays = []
        for _, points in lidar_infos.items():
            # Add valid element
            points = np.hstack((points, np.ones((points.shape[0], 1))))
            # Pad or truncate along the first dimension
            if points.shape[0] < max_length:
                padded_points = np.vstack(
                    (points, np.zeros((max_length - points.shape[0], 4)))
                )
            else:
                padded_points = points
            reshaped_arrays.append(padded_points)

        lidar_points = torch.from_numpy(
            np.array(reshaped_arrays, dtype=np.float32)
        ).permute(
            1, 0, 2
        )  # shape: (num_points (1000s), num_steps (11), xyz_valid (3+1))

        center_objects = torch.from_numpy(center_objects)

        # Sample lidar points according to config
        lidar_points = lidar_points[:: self.dataset_cfg.get("LIDAR_SAMPLE", 10), :, :]

        # Add center_obj dimension and repeat everything else
        lidar_points = lidar_points[None, :, :, :].repeat(num_center_objects, 1, 1, 1)

        # Transform points for each center_obj
        lidar_points = transform_to_center_coordinates(lidar_points)

        # Use only present snapshot
        lidar_points = lidar_points[:, :, -1, :].squeeze(dim=2)

        # Convert back to numpy
        lidar_points = lidar_points.numpy()
