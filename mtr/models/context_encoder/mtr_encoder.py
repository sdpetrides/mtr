...
class MTREncoder(nn.Module):
    def __init__(self, config):
        ...
        self.use_performer = self.model_cfg.get("USE_PERFORMER", False)
        self_attn_layers = []
        for _ in range(self.model_cfg.NUM_ATTN_LAYERS):
            self_attn_layers.append(
                self.build_transformer_encoder_layer(
                    ...
                    local_window_size=self.model_cfg.get("NUM_OF_ATTN_NEIGHBORS", 16),
    ...
    def apply_local_attn(self, x, x_mask, x_pos, num_of_neighbors):
        ...
        batch_idxs_full = (
            torch.arange(batch_size).type_as(x)[:, None].repeat(1, N).view(-1).int()
        )  # (batch_size * N)
